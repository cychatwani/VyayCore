...
public interface BalanceRepository extends JpaRepository<Balance, UUID> {
    ...
    @Query(value = """
            INSERT INTO balances (id, group_id, user_id, currency_code, net_amount_minor, created_at, updated_at)
            SELECT u.id, u.gid, u.uid, u.cid, u.delta, now(), now()
            ...
            DO UPDATE SET net_amount_minor = balances.net_amount_minor + EXCLUDED.net_amount_minor,
                          updated_at = now()
            """, nativeQuery = true)
    void applyDeltasBatch(@Param("ids") UUID[] ids,
            @Param("groupIds") UUID[] groupIds,
            @Param("userIds") UUID[] userIds,
            @Param("currencyIds") UUID[] currencyIds,
            @Param("deltas") Long[] deltas);
